( /* eslint-disable complexity */ /* eslint-disable max-statements */ /* eslint-disable no-shadow-restricted-names */
  function(ns, undefined) {
    ...
    function syn__hasBindKeyAndIsFieldEffected(node, key, field) {
      return syn__hasBindKey(node, key) && syn__isFieldEffected(node, field)
    }
    ...
    function syn__isFieldEffected(node, field) {
      var fieldRegex = new RegExp('\\.' + __syn__regexFix(field) + '([^a-zA-Z0-9_]|$)')
      if (node.hasAttribute(META_IF)) {
        if (node.getAttribute(META_IF).match(fieldRegex)) {
          return true
        }
      }
      var childrenWithIf = node.querySelectorAll('[' + META_IF + ']')
      for (var idx = 0; idx < childrenWithIf.length; ++idx) {
        var childWithIf = childrenWithIf[idx]
        if (childWithIf.getAttribute(META_IF).match(fieldRegex)) {
          return true
        }
      }
      if (elements__template(node).match(fieldRegex)) {
        return true
      }
      return false
    }
    ...
    function __binds__onDetailChange(ignored, resource, data) {
      ...
      var key = resource.key,
        toEval = __binds__fieldEffected(key, data),
        evaled = __binds__clone(toEval)
      for (var field in data) {
        if (data.hasOwnProperty(field)) {
          Array.prototype.push.apply(evaled, __binds__showFieldEffected(key, field))
        }
      }
    ...
    function __binds__fieldEffected(key, fields) {
      ...
      potentials.forEach(function(element) {
        ...
        for (var field in fields) {
          if (syn__isFieldEffected(element, field)) {
            console.info('Setu.binds @ bound-n-effected', key, field, element, emeta.$p, emeta.$p.$t)
    ...
    function __binds__showFieldEffected(key, field) {
      var elements = [],
        hidden = []
      observer__stop()
      hidden__all().forEach(function(comment) {
        ...
        var temp = elements__tempFromComment(comment),
          emetaComment = emeta__get(comment),
          emeta = emeta__new(temp)
        emeta.$p = emetaComment.$p
        if (syn__hasBindKeyAndIsFieldEffected(temp, key, field)) {
      ...
      observer__monitor(GAppElement)
      console.info('Setu.parser $ unhide', key, field, hidden, elements)
      return elements
    }
